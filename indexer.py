...
def index_from_db():
  cursor = db.cursor()
  cursor.execute("SELECT id, html FROM rawhtml A, posting_location B WHERE (B.alt_prices IS NULL OR B.alt_quantities IS NULL) AND active = 1 AND A.id = B.location_fk")
  print "here"
