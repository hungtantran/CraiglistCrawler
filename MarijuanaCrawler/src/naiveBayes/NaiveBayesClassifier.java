...
public class NaiveBayesClassifier {
	...
	public NaiveBayesClassifier(ModelTrainer model, FeaturesCalculator cal,
		...
		int[] numFeatureOccurencePositive = model
				.getNumFeatureOccurencePositive();
		int[] numFeatureOccurenceNegative = model
		...
		System.out.println(this.featureVectorSize + " " + this.probPositive
				+ " " + this.probNegative);

		// Use Laplace smoothing
		this.probFeatureGivenNegative = new double[this.featureVectorSize];
		for (int i = 0; i < this.featureVectorSize; i++) {
			this.probFeatureGivenNegative[i] = (double) (numFeatureOccurenceNegative[i] + 1)
					/ (numNegativeEntries + 2);
			// System.out.println(this.probFeatureGivenNegative[i]);
		}

		this.probFeatureGivenPositive = new double[this.featureVectorSize];
		for (int i = 0; i < this.featureVectorSize; i++) {
			this.probFeatureGivenPositive[i] = (double) (numFeatureOccurencePositive[i] + 1)
					/ (numPositiveEntries + 2);
